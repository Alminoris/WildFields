...
public class CoyoteEntity extends TameableEntity implements GeoEntity, Angerable
{
    private final AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    private static final TrackedData<Integer> ANGER_TIME = DataTracker.registerData(CoyoteEntity.class, TrackedDataHandlerRegistry.INTEGER);
    ...
    public CoyoteEntity(EntityType<? extends TameableEntity> entityType, World world)
    {
    ...
    protected void initGoals()
    {
        ...
        this.goalSelector.add(1, new CoyoteEscapeDangerGoal(1.5));
    ...
    protected SoundEvent getAmbientSound()
    {
        ...
        {
            return ModSounds.SOUND_COYOTE_GROWL;
        }
        else if (this.random.nextInt(3) == 0)
        {
            return this.isTamed() && this.getHealth() < 20.0F ? ModSounds.SOUND_COYOTE_WHINE : ModSounds.SOUND_COYOTE_AMBIENT;
        }
        else
        {
            return ModSounds.SOUND_COYOTE_AMBIENT;
        }
    }
    ...
    {
        return ModSounds.SOUND_COYOTE_HURT;
    }

    @Override
    protected SoundEvent getDeathSound()
    {
        return ModSounds.SOUND_COYOTE_DEATH;
    }
    ...
    public ActionResult interactMob(PlayerEntity player, Hand hand)
    {
        ...
        else if (itemStack.isOf(ModItems.JACKRABBIT))
        {
            if (!player.getAbilities().creativeMode)
            {
                itemStack.decrement(1);
    ...
    {
        CoyoteEntity coyoteEntity = ModEntities.COYOTE.create(serverWorld);
        if (coyoteEntity != null && passiveEntity instanceof CoyoteEntity coyoteEntity2)
        {
            if (this.isTamed())
            {
                coyoteEntity.setOwnerUuid(this.getOwnerUuid());
    ...
    public boolean canBreedWith(AnimalEntity other)
    {
        if (other == this)
            ...
            return false;
        else if (!(other instanceof CoyoteEntity coyoteEntity))
            return false;
        else if (!coyoteEntity.isTamed())
            return false;
        else
            return !coyoteEntity.isSleeping() && !coyoteEntity.isInSittingPose() && this.isInLove() && coyoteEntity.isInLove();
    }
    ...
    public boolean canAttackWithOwner(LivingEntity target, LivingEntity owner)
    {
        ...
        else if (target instanceof CoyoteEntity coyoteEntity)
        {
            return !coyoteEntity.isTamed() || coyoteEntity.getOwner() != owner;
        }
        else
        {
    ...
    class CoyoteEscapeDangerGoal extends EscapeDangerGoal
    {
        public CoyoteEscapeDangerGoal(double speed) {
            super(CoyoteEntity.this, speed);
        }
